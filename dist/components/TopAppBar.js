...
})(({ theme, drawerOpen, drawerWidth, overlayDrawer }) => ({
...
export default function TopAppBar({ defaultTitle, drawerWidth = 240, hasDrawer = true, showThemeToggle = true, actions, overlayDrawer = false }) {
    const { mode, toggleMode, drawerCollapsed, setDrawerCollapsed, topBarTitle, topBarChildren, } = useRootLayoutContext();
    return (_jsx(AppBar, { position: "fixed", drawerOpen: hasDrawer && !drawerCollapsed && !overlayDrawer, drawerWidth: drawerWidth, overlayDrawer: overlayDrawer, children: _jsxs(Toolbar, { children: [hasDrawer && (_jsx(Tooltip, { title: "Expandir/recolher menu", children: _jsx(IconButton, { color: "inherit", edge: "start", onClick: () => setDrawerCollapsed(!drawerCollapsed), sx: {
