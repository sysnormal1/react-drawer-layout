...
export default function TopAppBar({ title, drawerWidth = 240, hasDrawer = true, actions, showThemeToggle = true, }) {
    const { mode, toggleMode, drawerCollapsed, setDrawerCollapsed } = useRootLayoutContext();
    return (_jsx(AppBar, { position: "fixed", drawerOpen: hasDrawer && !drawerCollapsed, drawerWidth: drawerWidth, children: _jsxs(Toolbar, { children: [hasDrawer && (_jsx(Tooltip, { title: "Expandir/recolher menu", children: _jsx(IconButton, { color: "inherit", edge: "start", onClick: () => setDrawerCollapsed(!drawerCollapsed), sx: {
                        ...
                        }, children: _jsx(MenuIcon, {}) }) })), _jsx(Typography, { variant: "h6", sx: { flexGrow: 1 }, children: title }), actions, showThemeToggle && (_jsx(Tooltip, { title: `Mudar para tema ${mode === 'light' ? 'escuro' : 'claro'}`, children: _jsx(IconButton, { color: "inherit", onClick: toggleMode, children: mode === 'light' ? _jsx(DarkMode, {}) : _jsx(LightMode, {}) }) }))] }) }));
}
