...
function DrawerItemRow({ item, collapsed, currentPath, onNavigate, searchQuery, depth = 0, }) {
    ...
    const button = (_jsxs(ListItemButton, { onClick: handleClick, selected: isActive, sx: { pl: 2 + depth * 2 }, children: [item.icon && (_jsx(ListItemIcon, { sx: { minWidth: collapsed && depth === 0 ? 'unset' : 36 }, children: item.icon })), !collapsed && (_jsxs(_Fragment, { children: [_jsx(ListItemText, { primary: item.label }), hasChildren && (open ? _jsx(ExpandLess, {}) : _jsx(ExpandMore, {}))] }))] }));
...
export function DrawerItemList({ items, collapsed, currentPath, onNavigate, searchQuery, }) {
    const filtered = items.filter(item => matchesSearch(item, searchQuery ?? ''));
    return (_jsx(List, { disablePadding: true, children: filtered.map(item => (_jsx(DrawerItemRow, { item: item, collapsed: collapsed, currentPath: currentPath, onNavigate: onNavigate, searchQuery: searchQuery }, item.id))) }));
}
